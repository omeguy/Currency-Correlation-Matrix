...
class MarketStatus:
    ...
    def check_market_open(self):
        ...
        elif current_day_of_week == 6 and current_hour < 23:
            return False
...
class DataFetcher:
    ...
    def fetch(self):
        try:
            self.data = pd.DataFrame(mt5.copy_rates_from_pos(self.symbol, self.timeframe, self.from_data, self.to_data))
            print(f"[{datetime.now()}] Data fetched -- successful {self.symbol}")
    ...
    def get_current_price(self):
        # Fetch the latest market data and convert it to a DataFrame
        market_data = pd.DataFrame(mt5.copy_rates_from_pos(self.symbol, self.timeframe, 0, 1))
        ...
        # Assuming we're interested in the closing price
...
class Position:
    def __init__(self, symbol, trade_type, lot, open_price, magic_number, ticket_id, open_time, stop_loss, take_profit):
        self.symbol = symbol
        self.trade_type = trade_type
        self.lot = lot
    ...
    def close(self, close_time, close_price):
        self.status = "closed"
...
class TradeHistory:
    ...
    def get_history(self, ticket):
        ...
        history_orders = mt5.history_orders_get(ticket, group=self.symbol)
